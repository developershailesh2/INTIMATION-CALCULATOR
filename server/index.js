...
app.get('/get-details/:_id',(req , res) => {
    mongoClient.connect(connectionString).then((clientObj)=> {
        var database = clientObj.db("intimation-calculator");
        database.collection("client-amount").find({_id : new ObjectId(req.params._id)}).toArray().then((documents)=>{
            console.log(documents);
            res.send(documents);
            res.end();
        });
...
app.put('/edit-detail/:_id',(req , res) => {
    mongoClient.connect(connectionString).then((clientObj) => {
        ...
        database.collection('client-amount')
...
app.delete('/delete-client/:_id',(req , res) => {
    mongoClient.connect(connectionString).then((clientObj) => {
        var database = clientObj.db("intimation-calculator");
        database.collection('client-amount').deleteOne({ _id : new ObjectId(req.params._id)}).then((document)=>{
        ...
        }).catch(error => {console.error('Error in deleting client',error)});
    })
})
